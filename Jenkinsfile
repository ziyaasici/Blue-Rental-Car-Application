            ...
            steps {
                dir("Solution-Files/Task1/Terraform") {
                    script {
                        ...
                        sh "aws ec2 create-key-pair --key-name ${params.Environment}-Keypair --region ${AWS_REGION} --query 'KeyMaterial' --output text > ${params.Environment}-Keypair.pem"
                        sh "sudo chmod 400 ${params.Environment}-Keypair.pem"
